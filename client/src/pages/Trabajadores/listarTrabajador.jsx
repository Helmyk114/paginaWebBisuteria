  ...
  const eliminarTrabajador = async (idCardWorker) => {
    try {
      ...
      if (result.isConfirmed) {
        ...
        const nuevaInformacion = informacion.filter(
          (datos) => datos.idCardWorker !== idCardWorker
        );
            ...
            informacion.map((datos) => (
              <>
                <CardPerfil
                  ...
                  alignItems={"center"}
                >
                  <Avatares src={`${urlImage}/${datos.photo}`} radio={"full"} />
                  <div style={{ display: "block" }}>
                    <Texto1Card texto={"Numero de cuenta"} />
                    <Texto2Card texto2={"hla"} />
                    <Texto2Card texto2={"hola"} />
                  </div>
                  ...
                  <div className="relative flex items-center gap-1">
                    ...
                    <Tooltip content="Eliminar trabajador">
                      <span className="text-lg text-danger cursor-pointer active:opacity-50">
                        <DeleteIcon
                          eliminar={() =>
                            eliminarTrabajador(datos.idCardWorker)
                          }
                        />
                      </span>
                    </Tooltip>
